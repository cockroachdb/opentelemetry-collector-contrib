...
func TestHTTPClientRejectsDNSRebinding(t *testing.T) {
	...
	wrap, err := extension.GetHTTPRoundTripper(t.Context())
	require.NoError(t, err)
	transport, err := wrap(t.Context(), &http.Transport{
...
func TestGetHTTPRoundTripperHardensTransport(t *testing.T) {
	...
	wrap, err := extension.GetHTTPRoundTripper(t.Context())
	require.NoError(t, err)
	roundTripper, err := wrap(t.Context(), base)
	require.NoError(t, err)
...
func TestGetHTTPRoundTripperRequiresInnermostPosition(t *testing.T) {
	...
	wrap, err := extension.GetHTTPRoundTripper(t.Context())
	require.NoError(t, err)
	_, err = wrap(t.Context(), roundTripperFunc(func(*http.Request) (*http.Response, error) {
		return nil, nil
	}))
...
func TestGetHTTPRoundTripperRequiresTLSVerification(t *testing.T) {
	...
	wrap, err := extension.GetHTTPRoundTripper(t.Context())
	require.NoError(t, err)
	_, err = wrap(t.Context(), &http.Transport{
